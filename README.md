...
You add one small watchdog workflow to your repo on its own cron schedule (every 30 minutes works well). Each run, jobwatch reads the `on: schedule:` cron lines from every workflow file in the repo, checks each workflow's recent run history with your repo token, and sends an alert when:
...
3. Commit this file as `.github/workflows/jobwatch.yml`:
...
For finer control, commit `.github/jobwatch.yml` to the monitored repo. Keys are workflow filenames:
...
```
jobwatch: scheduled run failed
nightly-backup.yml failed at Aug 22, 02:00 UTC.
https://github.com/abhishekshree/jobwatch/actions/runs/1234567890
```

## Limitations

- No memory between runs. While something stays broken, jobwatch re-alerts on every check until it is fixed. That nagging is intentional; silence should mean everything passed.
...
- The monitored repo must be checked out first (the quick-start workflow above already does this), since jobwatch reads its workflow files from disk.
